const raw = `daegb gadbcf cgefda edcfagb dfg acefbd fdgab fg bdcfa fcgb | cdfgba fgbc dbfac gfadbc
...
/*const raw = `be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
function findCommon(str, tbl) {
  let r = '';
  for (let i = 0; i < str.length; ++i) {
    const check = tbl.map((v) => v.includes(str[i]));
    if (check.every((v) => v)) {
      r += str[i];
    }
  }
...
function getNumber(n, a, b, c, d, e, f, g) {
  if (match(n, [c, f])) {
    return 1;
  } else if (match(n, [a, c, d, e, g])) {
    return 2;
  } else if (match(n, [a, c, d, f, g])) {
    return 3;
  } else if (match(n, [b, c, d, f])) {
    return 4;
  } else if (match(n, [a, b, d, f, g])) {
    return 5;
  } else if (match(n, [a, b, d, e, f, g])) {
    return 6;
  } else if (match(n, [a, c, f])) {
    return 7;
  } else if (match(n, [a, b, c, d, e, f, g])) {
    return 8;
  } else if (match(n, [a, b, c, d, f, g])) {
    return 9;
  }
  return 0;
}
...
const data = raw
  .split('\n')
  .map((v) => v.split(' | '))
  .map((v) => [v[0].split(' '), v[1].split(' ')]);

const inputs = data.map((v) => v[0]);
...
for (let i = 0; i < inputs.length; ++i) {
  ...
  let a, b, c, d, e, f, g;
  a = value[7].replace(value[1][0], '').replace(value[1][1], '');
  const adg = findCommon(value[5][0], [value[5][1], value[5][2]]);
  const dg = adg.replace(a, '');
  const bfg = findCommon(value[6][0], [value[6][1], value[6][2]]);
  g = findCommon(dg, [bfg]);
  d = dg.replace(g, '');
  f = findCommon(value[1], [bfg]);
  c = value[1].replace(f, '');
  b = value[4].replace(c, '').replace(d, '').replace(f, '');
  e = value[8]
    .replace(a, '')
    .replace(b, '')
    .replace(c, '')
  ...
  for (let o = 0; o < output.length; ++o) {
    n += Math.pow(10, 3 - o) * getNumber(output[o], a, b, c, d, e, f, g);
  }
  console.log(n);
  result += n;
}
